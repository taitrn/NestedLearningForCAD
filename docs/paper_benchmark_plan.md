...
Use the Phase 1-2 anchor run, the conservative Phase 3 after-eval run, and the
max-power after-eval run as separate rows only if the report clearly explains
...
3. A Markdown or LaTeX benchmark table generated from local rows.
4. A short protocol paragraph in the report.
5. A limitation paragraph distinguishing demo validation, local benchmark rows,
